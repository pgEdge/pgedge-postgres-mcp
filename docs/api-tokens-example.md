...
#   - Create new token:  ./bin/pgedge-nla-server -add-token
#   - List all tokens:   ./bin/pgedge-nla-server -list-tokens
#   - Remove a token:    ./bin/pgedge-nla-server -remove-token <id-or-hash-prefix>
#
...
# $ ./bin/pgedge-nla-server -http -auth-mode per-token
